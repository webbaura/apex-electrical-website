...
export default function Services() {
  const pageData   = content.pages?.services || {};
  const { headline, subheadline } = pageData;
  // Use top-level services[] (all items) with pages.services.items as fallback
  const items = (content.services?.length ? content.services : pageData.items) || [];
  return (
    <>
            ...
            {(items ?? []).map((s, i) => (
              <Animate key={i} animation="fade-up" delay={i * 60}>
                <div className="card-hover group relative p-8 rounded-2xl border overflow-hidden h-full"
                  ...
                  <h2 style={{ color: 'var(--color-text)' }} className="text-2xl font-bold mb-3">{s.title}</h2>
                  <p style={{ color: 'var(--color-text-muted)' }} className="leading-relaxed mb-5">{s.description}</p>
